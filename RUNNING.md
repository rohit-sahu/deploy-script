...
### Cloudflare proxied DNS (orange-cloud) in front of nginx instead — `--cloudflare-proxied`

Different from the Tunnel modes above: this is for when you point your domain's DNS at this server through Cloudflare with the record proxied (orange cloud), and requests reach `nginx` directly over the internet (not via `cloudflared`). Without any changes, nginx would see every visitor as one of Cloudflare's own edge IPs (`$remote_addr` is whichever Cloudflare edge node proxied the request), breaking `ADMIN_IP_ALLOWLIST` and per-IP rate limiting.

```bash
./deploy.sh --cloudflare-proxied your-domain.com
```

This switches nginx to `nginx/https.cloudflare.conf.template`, which uses `ngx_http_realip_module` (restricted to Cloudflare's published ranges, `nginx/cloudflare-ips.conf`) to resolve `$remote_addr` back to the real visitor IP from the `CF-Connecting-IP` header. Remembered in `.env` like `DOMAIN` — a bare re-run keeps using it; `--no-cloudflare-proxied` turns it back off.

**You must also firewall ports 80/443 to Cloudflare's IP ranges** (e.g. `ufw`/security group rules restricted to https://www.cloudflare.com/ips/) — otherwise anyone can bypass Cloudflare entirely, hit this nginx directly, and forge their own `CF-Connecting-IP` header, defeating the allowlist/rate limiting. This flag only handles the nginx side; the firewall rule is a host-level step this script can't do for you.

Cloudflare's IP ranges change occasionally (rarely). Keep `nginx/cloudflare-ips.conf` current with:

```bash
npm run cloudflare-ips:update
```

Fetches the latest ranges from `https://www.cloudflare.com/ips-v4`/`-v6` (no dependencies, just Node's built-in `https`), rewrites `nginx/cloudflare-ips.conf` only if something actually changed, and reloads the running `nginx` container to pick it up immediately (falls back to "picks it up on next start" if `nginx`/`docker compose` isn't running/available). Use `npm run cloudflare-ips:update -- --check` to only detect drift (exit code `1` if an update is available) without writing anything — useful for a cron job/monitoring alert.

To automate it, add a periodic host crontab entry (weekly is plenty — Cloudflare almost never changes these):

```cron
0 3 * * 0 cd /path/to/deploy-script && npm run cloudflare-ips:update >> /var/log/cloudflare-ips-update.log 2>&1
```

Not needed if you're only using `--tunnel`/`--quick-tunnel` (those bypass nginx entirely — see the Client-IP trust model section below). You can combine `--cloudflare-proxied` with `--local`, though that's an unusual setup (self-signed cert + Cloudflare in front) mostly useful for testing this specific config path.
...
Follow the prompts (email, password, confirmation, and optionally 2FA — see below). Re-run it anytime to add another admin, change a password, or change 2FA settings — it upserts by email and takes effect immediately, no restart needed (both `npm run dev` and the Docker container read the file live).

To remove an admin, edit `secrets/admin-users.json` directly and delete their entry.

## Admin login protections

`/admin` has three independent layers of protection beyond the password itself, all optional/off-by-default except rate limiting:

- **Rate limiting** — always on. 5 failed login attempts (per email or per IP) within 15 minutes locks that key out for 15 minutes. Tracked in MongoDB (TTL-indexed, survives restarts); fails open (never locks everyone out) if MongoDB is briefly unavailable.
- **2FA (TOTP)** — opt-in per admin via `npm run admin:create` ("Enable 2FA?" prompt). Works with Google Authenticator, Authy, 1Password, Bitwarden, or any standard RFC 6238 app. Requires `ADMIN_SECRETS_KEY` to be set (auto-generated by `npm run env:create`) — the secret is AES-256-GCM-encrypted at rest in `secrets/admin-users.json`, never stored in plaintext.
- **IP allowlist** — set `ADMIN_IP_ALLOWLIST` (comma-separated IPs/CIDRs) to restrict `/admin/*` to specific networks, enforced in the app's edge middleware before any auth check runs.

### Client-IP trust model (important if you customize the proxy setup)

The app determines "the client's IP" (for both the allowlist and rate limiting) by checking, in order: `CF-Connecting-IP` (if present — set only by Cloudflare's edge, for both proxied DNS and Tunnel, and not spoofable by the client), then the left-most entry of `X-Forwarded-For`, then `X-Real-IP`.

This is safe **only** because:
- The `web` container is never reachable except through `nginx` or `cloudflared` (`expose`, not `ports`, in `docker-compose.yml`) — there's no way to hit it directly and inject a fake header.
- `nginx/https.conf.template` (default) sets `X-Forwarded-For` to `$remote_addr` (overwriting, not appending-to, whatever a client sent) — see the comment in that file if you ever modify it.
- `nginx/https.cloudflare.conf.template` (used when `--cloudflare-proxied`/`CLOUDFLARE_PROXIED=1`) instead trusts `CF-Connecting-IP` via `ngx_http_realip_module`, restricted to Cloudflare's own published ranges — safe only when 80/443 are ALSO firewalled to those same ranges (see above), so nothing else can reach nginx and forge that header.
- Cloudflare's edge (not `cloudflared`) sets `CF-Connecting-IP` for the Tunnel modes too, stripping any client-supplied value first — this is why the app prefers that header over `X-Forwarded-For` regardless of which ingress path (nginx or `cloudflared`) a given request took.

If you put another reverse proxy or CDN in front of this stack, make sure it does the same (overwrite, don't append/trust client-supplied forwarding headers) — otherwise both the IP allowlist and rate limiting can be trivially bypassed.
...
```
```
dist/create-admin.mjs
dist/create-env.mjs
dist/create-cloudflare-tunnel-token.mjs
```
Run them directly, without `node_modules`:
```bash
node dist/create-admin.mjs
node dist/create-env.mjs -- local
node dist/create-cloudflare-tunnel-token.mjs
```

**Dev build** (unminified, sourcemaps, rebuilds automatically on every save):
```bash
npm run build:dev
```
